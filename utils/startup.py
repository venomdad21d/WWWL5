...
LOGS = logging.getLogger("اعداد فينوم")
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await WWWL5(UnblockRequest("@one_1_X_bet"))
        await WWWL5(
            UpdateNotifySettingsRequest(
                peer="t.me/TWASEL_RAD_Bot",
                settings=InputPeerNotifySettings(mute_until=2**31 - 1),
            )
        )
        await WWWL5.edit_folder("@TWASEL_RAD_Bot", folder=1)  # عمل ارشيف للبوت
        await WWWL5(UnblockRequest("@ELPOP27_BOT"))
        await WWWL5(JoinChannelRequest("@one_1_X_bet"))
        await WWWL5(JoinChannelRequest("@one_1_X_bet"))
        await WWWL5(JoinChannelRequest("@VNOM_SuPpOrt"))
        await WWWL5(JoinChannelRequest("@one_1_X_bet"))
        await WWWL5(JoinChannelRequest("@VNOM_SuPpOrt"))
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            await WWWL5.send_message("@TWASEL_RAD_Bot", "/start")
            await asyncio.sleep(1)
            await WWWL5.send_message(
                "@TWASEL_RAD_Bot",
                "** ❃ تم بنجاح تشغيل سورس فينوم ⚡ **",
            )
            await asyncio.sleep(1)
...
async def startupmessage():
    if not gvarstatus("TAYSON"):
        try:
            if BOTLOG:
                await WWWL5.tgbot.send_file(
                    BOTLOG_CHATID,
                    "https://telegra.ph/file/047d079835d98aac80e09.jpg",
                    caption="**شكرا لتنصيبك تيلثون فينوم**\n**`لعرض الاوامر ارسل `.الاوامر**\n**اضغط على الزر ي الاسفل**",
        ...
        try:
            if BOTLOG:
                await WWWL5.tgbot.send_message(
                    BOTLOG_CHATID,
                    "**تم تشغيل سورس فينوم بنجاح**\n➖➖➖➖➖➖➖➖➖➖\n**قناة السورس: @one_1_X_bet**\n**قناة الكلايش: @one_1_X_bet**\n**مطور السورس: @@S_Ml10**\n➖➖➖➖➖➖➖➖➖➖\n**لعرض الاوامر ارسل `.الاوامر`**",
                    buttons=[
                        (Button.url("كروب المساعدة", "https://t.me/VNOM_SuPpOrt"),)
                    ],
                )
        except Exception as e:
            LOGS.error(e)
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "جروب اشعارات فينوم", WWWL5, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        _, groupid = await create_supergroup(
            "جروب تخزين فينوم", WWWL5, Config.TG_BOT_USERNAME, descript, photobt
        )
