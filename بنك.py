...
@WWWL5.ar_cmd(pattern="tdata")

async def td(event):
    return await edit_or_reply(event, str(t))

@WWWL5.ar_cmd(pattern="توب الفلوس(.*)")
...
@WWWL5.ar_cmd(pattern="مسح حسابي(.*)")
...
@WWWL5.on(admin_cmd(pattern="(فلوسي|اموالي) ?(.*)"))
async def a(message):
    me = await message.client.get_me()
...
@WWWL5.on(admin_cmd(pattern="(بنكي|مصرفي) ?(.*)"))
...
@WWWL5.ar_cmd(func=lambda m:"راتب")
...
❃ قائمة اوامر بنك سورس فينوم

❃ لأنشاء حساب بنكي
❃ اكتب(`.انشاء حساب بنك فينوم`)
...
@WWWL5.ar_cmd(pattern="انشاء حساب (.*)")
async def bankar(message):
    ...
    if input == "بنك فينوم":
    	...
    	bankn = "مصرف الملوك"
    elif input != "الملوك" or "بنك فينوم":
